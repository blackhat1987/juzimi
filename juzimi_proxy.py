...
def getJuzi(num,ip):
	...
	juzi = re.search(pattern_juzi, html).group(1).replace(r'<br/>', '      ').replace(r'&quot;',r'"').replace('\r','')
	try:
...
if __name__=="__main__":
	...
	num = getCount()
	print num
	for i in range(8): #设置访问数量
		try:
			num = int(getNum(num,ip))
			result = getJuzi(num,ip)
			print num, 'is ok'
			...
			if e.code==403:
				...
				print num,'netgate error',e.code
				#发送邮件提醒 拒绝访问
				clock = datetime.datetime.now()
				content = str(clock) + '<br/><br/>' + str(e.code)	# 邮件内容
				send_email = SendEmail(content, 'html')
			...
			#发送邮件提醒 代理服务器
			clock = datetime.datetime.now()
			content = str(clock) + '<br/><br/>' + '代理服务器不可用'	# 邮件内容
			send_email = SendEmail(content, 'html')
			send_email.send()
			sys.exit(0)
		...
		print code
		setCount(num)
		time.sleep(random.randint(6,10))	#设置间隔时间
	print 'exit......'
